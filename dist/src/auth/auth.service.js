...
const ALLOWED_SELF_REGISTER_ROLES = [
    register_dto_1.AccountType.USER,
    register_dto_1.AccountType.COACH,
];
let AuthService = AuthService_1 = class AuthService {
    ...
    async register(dto) {
        ...
        const requestedRole = dto.accountType ?? register_dto_1.AccountType.USER;
        if (!ALLOWED_SELF_REGISTER_ROLES.includes(requestedRole)) {
            throw new common_1.ForbiddenException('Invalid account type');
        }
        ...
        return {
            accessToken: this.generateToken(Number(user.id), user.email, Number(user.roleId)),
            user: {
                id: user.id,
                email: user.email,
    ...
    generateToken(userId, email, roleId) {
