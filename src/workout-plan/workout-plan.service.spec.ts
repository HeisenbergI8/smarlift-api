...
describe('WorkoutPlanService', () => {
        ...
        days: [
          {
            dayNumber: 1,
            name: 'Push',
            exercises: [{ exerciseId: BigInt(10) }],
          },
        ],
      });
  ...
  describe('findOne', () => {
    it('should return a plan by id', async () => {
      mockPrisma.workoutPlan.findUnique.mockResolvedValue(mockPlan);
  ...
  describe('getActivePlan', () => {
    it('should return the active plan', async () => {
      ...
      mockPrisma.workoutPlan.findFirst.mockResolvedValue(activePlan);
  ...
  describe('update', () => {
    it('should update plan metadata without replacing days', async () => {
      mockPrisma.workoutPlan.findUnique.mockResolvedValue(mockPlan);
      const updatedPlan = { ...mockPlan, name: 'Updated PPL' };
      mockTx.workoutPlan.update.mockResolvedValue(updatedPlan);
  ...
  describe('activate', () => {
    it('should deactivate all plans and activate the specified one', async () => {
      ...
      mockTx.workoutPlan.update.mockResolvedValue(activatedPlan);
